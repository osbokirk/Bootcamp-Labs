...
public class Sandwhich implements Sellable {
    ...
    public void setCheeses(HashMap<String, Double> cheeses) {
    ...
    public void setSides(HashMap<String, Double> sides) {
        this.sides = sides;
    }

    public Sandwhich buildTest(){
        //Sandwhich sammmy = new Sandwhich();
        return new Sandwhich(SandwhichSize.valueOf("FOURINCH"),"White");
    }

    public Sandwhich(SandwhichSize size, String bread) {
        this.size = size;
        this.bread = bread;
    ...
    public double getPrice() {
        double total  = size.getBase();
        int meatCount = (int) this.meats.values().stream().mapToDouble(value -> value).sum();
        if(meatCount >= 1){total += size.getMeat() + (meatCount - 1) * size.getExtraMeat();}
        int cheeseCount = (int) this.cheeses.values().stream().mapToDouble(value -> value).sum();
        if(cheeseCount >= 1){total += size.getCheese() + (cheeseCount -1) * size.getExtraCheese();}
        return total;
    }
    public String toString(){
        String string =size.toString()+"     "+getPrice()+"\n";
